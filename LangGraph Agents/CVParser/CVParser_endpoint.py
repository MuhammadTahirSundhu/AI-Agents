...
def extract_pdfs_to_markdown(shared_link: str, api_key: str = None) -> List[Dict[str, str]]:
    """
    Streams all PDF files from a publicly accessible Google Drive folder and extracts their content to markdown.
    
    Args:
        shared_link (str): The Google Drive shared folder link
    ...
    def extract_pdf_advanced(pdf_stream: BytesIO, filename: str) -> str:
        try:
    ...
    def extract_pdf_simple(pdf_stream: BytesIO, filename: str) -> str:
        try:
            doc = fitz.open(stream=pdf_stream, filetype="pdf")
            text_content = []
...
def GetThingsFromInput(input_query: str) -> list:
    ...
    - For folder_path: Look for any file paths, directory references, or storage locations (like https://drive.google.com/drive/folders/14VrlgFXFXxRWgtsLKxjIin2UPGxf61GW etc)
...
- Strictly follow Format: "I'll now analyze the CVs with these parameters: Job Description: [summary], Top K: {topk_value}, Folder: {folder_path}"
...
def get_cvs_data(folder_path: str) -> str:
    ...
    # extracted_contents = run_extractor(folder_path)
